...
- ✅ `MAIRU-012 / #12` を完了し、Export 画面、分類/処理ログ出力、blocklist JSON 取込、重要メール PDF、`mbox` 調査メモを追加済み。
- ✅ `MAIRU-013 / #13` を完了し、定期実行スケジューラー基盤、再試行制御（最大3回・指数バックオフ）、重複実行防止を実装済み。
- ✅ `MAIRU-014 / #14` を完了し、OS 通知と自動実行設定 UI（通知 ON/OFF、実行間隔設定）を実装済み。
...
## 次のアクション
1. Phase 4 の未完タスク（新着メール分類の日次実行、既知ブロック送信者の 30 分処理）を後続 issue で実装する。
2. `MAIRU-015 / #35` に進み、移行アシスタントとエクスポート導線の接続を実装する。

...
## Issue 運用
- 実装に着手する前に `docs/ISSUES.md` から対象 issue を選び、依存関係を確認してください。
- ブランチ名、コミットメッセージ、PR 説明には `MAIRU-004` のようなローカル issue ID を含めます。
- GitHub Issue を作成する場合は `.github/ISSUE_TEMPLATE/task-from-backlog.md` を使い、タイトル先頭にローカル issue ID を残します。
- 着手した issue は GitHub 上で `status: in progress` にし、担当者を assignee に設定します。
- 完了した issue は `.github/ISSUE_CLOSE_COMMENT_TEMPLATE.md` でコメントを残し、GitHub 上で `status: done` にしてからクローズします。
- `docs/TASKS.md` は全体計画、`docs/ISSUES.md` は日々の実装単位として使い分けます。
